...
btnnormal.onclick = function DistribuicaoNormal(){
    ...
    let DesvioPadrao = Number(document.getElementById("dp").value);
    let calculo = document.getElementById("selectCalculo").value;
    let probabilidade;

    if(calculo == "Maior que"){
        let elementoPesquisado = Number(document.getElementById("valor").value);
        let valor = EncontrarValorNaTabela(media, DesvioPadrao, elementoPesquisado);
        if(elementoPesquisado < media){
            probabilidade = ((valor + 0.5) * 100 ).toFixed(2) + "%";
        }else{
            probabilidade = ((0.5 - valor) * 100).toFixed(2) + "%";
        }

    }else if(calculo == "Entre"){
            let elementosPesquisado =  (document.getElementById("valor").value).split(" ");
            elementosPesquisado = elementosPesquisado.map(a => Number(a));
            elementosPesquisado = elementosPesquisado.sort((a,b) => a - b);

            if((elementosPesquisado[0] < media && elementosPesquisado[1] < media) || (elementosPesquisado[0] > media && elementosPesquisado[1] > media)){
                let valor1 = EncontrarValorNaTabela(media, DesvioPadrao, elementosPesquisado[0]);
                let valor2 = EncontrarValorNaTabela(media, DesvioPadrao, elementosPesquisado[1]);
                probabilidade = ((valor2 - valor1) * 100).toFixed(2) + "%";

            }else{
                let valor1 = EncontrarValorNaTabela(media, DesvioPadrao, elementosPesquisado[0]);
                let valor2 = EncontrarValorNaTabela(media, DesvioPadrao, elementosPesquisado[1]);
                probabilidade = ((valor1 + valor2) * 100).toFixed(2) + "%";
            }
    }else if( calculo == "Menor que"){
        let elementoPesquisado = Number(document.getElementById("valor").value);
        let valor = EncontrarValorNaTabela(media, DesvioPadrao, elementoPesquisado);
        if(elementoPesquisado < media){
            probabilidade = ((0.5 - valor) * 100 ).toFixed(2) + "%";
...
function EncontrarValorNaTabela(media, desviopadrao, valor){
    let scoreZ = CalcularScoreZ(media, desviopadrao, valor);
    let linha = Math.floor(scoreZ * 10);
    let coluna = (((scoreZ * 10) - linha ) * 10).toFixed();
    let resultado = LocalizarCoordenada(linha, coluna);
    return resultado;
}

function CalcularScoreZ(media, desviopadrao, valor){
    let scoreZ = (valor - media) / desviopadrao;
    if(scoreZ < 0) scoreZ = scoreZ * -1;
    return Number(scoreZ.toFixed(2));
}

function LocalizarCoordenada(linha, coluna){
    const TabProbCurvaNormalReduzida = [
    ...
    return TabProbCurvaNormalReduzida[linha][coluna];
}